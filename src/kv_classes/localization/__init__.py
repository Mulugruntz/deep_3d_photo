...
Copied from https://raw.githubusercontent.com/fossasia/kniteditor/bc92b74128864dd85b52a25bf67e0a4ea0dfd9fe/kniteditor/localization/__init__.py
"""
import os
import gettext
...
_locales = None
DOMAIN = "Deep3DPhoto"  #: the name of the .po files

_languages_locales = gettext.translation(DOMAIN, _locale_dir,
...
for folder_name in os.listdir(_locale_dir):
    ...
    if os.path.isdir(folder):
        if "LC_MESSAGES" in os.listdir(folder):
            _supported_languages.add(folder_name)
            ll = folder_name.split('_')
            if ll[0] != 'en':
                _supported_languages.add(ll[0])
            del ll
...
def list_languages():
    ...
    :return: a set of :class:`strings <str>` of language codes that can be
...
DEFAULT_LANGUAGE = "en_GB"  #: the default language to use
